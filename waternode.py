...
STATUS_FAILSAFE_TIMER = 0
STATUS_TEMP = 1
STATUS_TANK_LEVEL = 2
STATUS_VALVE_ON = 3
STATUS_PUMPING = 4
STATUS_VALVE_CURRENT = 5
STATUS_PUMP_CURRENT = 7
STATUS_NUM_OF_STATUSES = 7

STATUS_ERROR = 255

FULL_TANK = 90 #90 inches is a full tank, ie 2800 gallons per tank
    ...
    def __init__(self,
        ...
        # got byte_array.  If a settings or status packet, populate opcode, command, and value
            ...
            if self.pl_type == PAYLOAD_TYPE_SETTINGS or self.pl_type == PAYLOAD_TYPE_STATUS:
                ...
                if self.command == STATUS_ERROR:
                    log.debug("Got error status")
                log.debug("opcode: {} | command: {}, value: {}".format(self.opcode, self.command, self.value))
            self.valid = True
...
class WaterNode(object):
    ...
    def __init__(self, addr=0x01, src_addr=0x00, ser=None):
        ...
        self._last_valve_turn_off_time = 0
        self._last_valve_turn_on_time = 0
        self._curr_valve_state = 0


        if not ser:
            self.ser = serial.Serial("/dev/ttyUSB0", baudrate=9600, timeout=0.2)
    ...
    def get_temp(self):
        ...
        log.error("Could not read current temperature. Invalid response")
        return None
    ...
    def get_tank_level(self):
        ...
        log.error("Could not read tank level. Invalid response")
        return None

    def get_valve_state(self):
        resp_pkt = self.read_status(STATUS_VALVE_ON)
        if resp_pkt.valid:
            if self._curr_valve_state == 1 and resp_pkt.payload.value == 0:
                self._last_valve_turn_off_time = time.time()
            self._curr_valve_state = resp_pkt.payload.value
            return self._curr_valve_state
        log.error("Could not read valve state. Invalid response")
        return None

    def get_last_valve_turn_off_time(self):
        return self._last_valve_turn_off_time

    def get_last_valve_turn_on_time(self):
        return self._last_valve_turn_on_time

    def get_pumping(self):
        resp_pkt = self.read_status(STATUS_PUMPING)
        if resp_pkt.valid:
            return resp_pkt.payload.value
        log.error("Could not read pumping state. Invalid response")
        return None

    def get_valve_current(self):
        resp_pkt = self.read_status(STATUS_VALVE_CURRENT)
        if resp_pkt.valid:
            return resp_pkt.payload.value
        log.error("Could not read valve current. Invalid response")
        return None
    ...
    def get_pump_current(self):
        ...
        if resp_pkt.valid:
            return resp_pkt.payload.value
        log.error("Could not read pump current. Invalid response")
        return None

    def set_valve_on(self):
        resp_pkt = self.write_status(STATUS_VALVE_ON, 1)
        if resp_pkt.valid:
            if resp_pkt.payload.value == 1:
                log.info("Valve is now on")
                if self._curr_valve_state == 0:
                    self._last_valve_turn_on_time = time.time()
                self._curr_valve_state = 1
                return self._curr_valve_state
            else:
                print("Failed to start turn on valve, response was: {}".format(resp_pkt.payload.value))
        else:
            log.error("Valve status unknown. Invalid response")

    def set_valve_off(self):
        resp_pkt = self.write_status(STATUS_VALVE_ON, 0)
        if resp_pkt.valid:
            if resp_pkt.payload.value == 0:
                log.info("Valve is now off")
                if self._curr_valve_state == 1:
                    self._last_valve_turn_off_time = time.time()
                self._curr_valve_state = 0
                return self._curr_valve_state
            else:
                print("Failed to turn valve off, response was: {}".format(resp_pkt.payload.value))
        else:
            log.error("Valve status unknown. Invalid response")
    ...
    def set_setting_tank_pump_turn_on_level(self, inches):
    ...
    def set_setting_tank_pump_turn_off_level(self, inches):
        tank_level = self._conv_inches_to_tank_level(inches)
        if tank_level < self.TANK_MIN_LEVEL_OFFSET:
            tank_level = self.TANK_MIN_LEVEL_OFFSET
    ...
    def _conv_temp_to_f(self, temp):
        # temperature is received in millivolts from node
    ...
    def read_status(self, status):
        log.debug("Creating packet to read status: {}".format(status))
    ...
    def _send_and_receive_packet(self, pkt):
        ...
        if resp_pkt.valid:
            ...
            log.error("Got invalid (or no) packet")
        return resp_pkt        
    ...
    def read_response(self):
        ...
        log.debug("read_response: {}".format(resp_arr))
        return resp_arr
